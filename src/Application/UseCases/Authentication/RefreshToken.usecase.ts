...
export class RefreshTokenUseCase implements IRefreshTokenUseCase{
    ...
    async execute(refreshToken: string): Promise<{ accessToken: string; refreshToken: string; }> {
        ...
        if(!user.isActive){
            throw new ProjectErrors(Http_StatusCodes.UN_AUTHORIZED,"ACCOUNT_DEACTIVATED",'Account is deactivated')
        }

        if(user.isSuspended){
            throw new ProjectErrors(Http_StatusCodes.UN_AUTHORIZED,"ACCOUNT_SUSPENDED",'Account has been suspended')
        }
